...
let GameRun = false, GameStarted = false;
let b1, b2;
let Modes;
let GriSOp;
//tuong duong diem tam thoi nguoi choi 1 va nguoi choi 2
let tempScoreOld = 0, tempScoreOne = 0;
let tTurn;
...
function draw() {
	...
	if (gameObject.scorePlays[0] != tempScoreOld || gameObject.scorePlays[1] != tempScoreOne) {
	...
	if (gameObject.GameStarted) {
		...
		if (GameRun) {
			...
			if (playerOption != 0) {
				...
				if (playerOption == 1 || playerOption == 2) { tTurn = 1; }
				else { tTurn = turn; }

				if (IJHi[2] == 0) {
					line(getSize + getSize * IJHi[0], getSize + getSize * IJHi[1], 2 * getSize + getSize * IJHi[0], getSize + getSize * IJHi[1]);
				} else {
					line(getSize + getSize * IJHi[0], getSize + getSize * IJHi[1], getSize + getSize * IJHi[0], 2 * getSize + getSize * IJHi[1]);
				}
			}
		}
		else {
...
function CheckNumberOfEgesOfRect(gameObject, i, j, hv) {
	...
	Nl[0] = gameObject.lines[r[0]][r[1]][0][0] + gameObject.lines[r[0]][r[1]][1][0] + gameObject.lines[r[0] + 1][r[1]][1][0] + gameObject.lines[r[0]][r[1] + 1][0][0];
	if (r.length > 2) {
		Nl[1] = gameObject.lines[r[2]][r[3]][0][0] + gameObject.lines[r[2]][r[3]][1][0] + gameObject.lines[r[2] + 1][r[3]][1][0] + gameObject.lines[r[2]][r[3] + 1][0][0];
	}
	return Nl
}
	...
	for (var i = 0; i < gameObject.rects.length - 1; i++) {
		for (var j = 0; j < gameObject.rects[i].length - 1; j++) {
			if (gameObject.rects[i][j][0] == 0) {
				if (gameObject.lines[i][j][0][0] + gameObject.lines[i][j][1][0] + gameObject.lines[i + 1][j][1][0] + gameObject.lines[i][j + 1][0][0] == 3) {
					if (gameObject.lines[i][j][0][0] == 0) {
						return [i, j, 0];
